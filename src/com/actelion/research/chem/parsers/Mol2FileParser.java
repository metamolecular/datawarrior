...
public class Mol2FileParser extends AbstractParser {
	...
	private void addMol(List<FFMolecule> res, FFMolecule m, Set<Integer> aromaticAtoms, Set<Integer> aromaticBonds) {
		...
		/*if(true) {
			...
			System.out.println(new SmilesCreator().generateSmiles(m.toStereoMolecule()));
			if(!suc) {
	...
	public void save(FFMolecule mol, int chargeType, Writer writer) throws Exception {
		save(Collections.singletonList(mol), chargeType, writer);
	}
	 ...
	 * @throws Exception
	 */
	public void save(List<FFMolecule> mols, int chargeType, Writer writer) throws Exception {
		...
		for (FFMolecule mol : mols) {
			...
			for (int i = 0; i < mol.getAllAtoms(); i++) {
				...
				writer.write(" ");
				writeL(writer, ExtendedMolecule.cAtomLabel[mol.getAtomicNo(i)] + (mol.isAromatic(i)?".ar":""), 8);
	...
	public static final ParserFileFilter FILEFILTER = new ParserFileFilter() {
		public boolean accept(File f) {
			return f.getName().toUpperCase().endsWith(".MOL2") || f.isDirectory();
		}
	...
	public static void main(String[] args) throws Exception {
		List<FFMolecule> res = new Mol2FileParser().loadGroup("c:/mopac11971.mol2");
		//List<FFMolecule> res = new Mol2FileParser().loadGroup("c:/orig_pose_min.mol2");
