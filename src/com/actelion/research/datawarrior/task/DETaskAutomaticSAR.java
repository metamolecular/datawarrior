...
public class DETaskAutomaticSAR extends DETaskAbstractAddChemProperty implements Runnable {
	...
	public DETaskAutomaticSAR(DEFrame parent, boolean isInteractive) {
		super(parent, DESCRIPTOR_NONE, false, false, isInteractive);
		mFrame = parent;
		}
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				if (getTableModel().getCategoryCount(firstNewColumn) <= 64) {
					...
					visualization.setSplittingColumns(firstNewColumn, JVisualization.cColumnUnassigned);
					}
				}
			} );
		}
