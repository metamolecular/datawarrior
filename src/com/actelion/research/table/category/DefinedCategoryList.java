...
public class DefinedCategoryList<T extends Comparable<? super T>> extends CategoryList<T> {
	private UniqueList<T> mDefinedOrder;

	public DefinedCategoryList(UniqueList<T> definedOrder, CategoryNormalizer<T> normalizer) {
	...
	public void add(T v) {
		if (!contains(v)) {
			int definedIndex = mDefinedOrder.getIndex(v);
			if (definedIndex == -1) {
			...
			definedIndex--;
			while (definedIndex >= 0 && !contains(mDefinedOrder.get(definedIndex)))
				definedIndex--;

			if (definedIndex == -1)
				((UniqueList<T>)getCategoryList()).add(0, v);
			else
				((UniqueList<T>)getCategoryList()).add(1+getIndex(mDefinedOrder.get(definedIndex)), v);
			}
		}
	}
